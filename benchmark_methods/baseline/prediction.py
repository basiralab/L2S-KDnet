...
class Baseline(object):
    """
    Build Baseline model for training and testing.
    """
    ...
    def build_model(self):
        """
        Build teachers and students, initialize optimizers.
        """
        ...
        param_list = [self.Teacher.parameters()]
        self.teacher_optimizer = torch.optim.Adam(itertools.chain(*param_list), self.opts.t_lr, [self.opts.beta1, self.opts.beta2])

        # build optimizer for students
        param_list = [self.Student.parameters()]
        self.student_optimizer = torch.optim.Adam(itertools.chain(*param_list), self.opts.s_lr, [self.opts.beta1, self.opts.beta2])
    ...
    def restore_model(self, resume_iters, model_name="teacher", fold=0):
        ...
        if model_name == "teacher":
            Teacher_path = os.path.join(self.opts.checkpoint_dir,
                                        'teacher-{}-{}-{}.ckpt'.format(fold, resume_iters,self.opts.local_topology_loss_weight))
            ...
            Student_path = os.path.join(self.opts.checkpoint_dir,
                                        'student-{}-{}-{}.ckpt'.format(fold, resume_iters,self.opts.local_topology_loss_weight))
    ...
    def show_mtrx(self, m, name):
        ...
        plot_path = os.path.join(self.opts.plot_dir,
                                 "{}_{}.png".format(name, self.opts.local_topology_loss_weight))
    ...
    def train(self, fold):
        ...
        for i in range(start_iters, self.opts.num_iters):
            ...
            # =================================================================================== #
            #                          2. Train the Teacher LR and SR                             #
            # =================================================================================== #

            adj = torch.eye(t0_M_encoder_T.shape[0]).to(self.device)
            embedding, F_fake_i = self.Teacher(t0_M_encoder_T, adj)

            teacher_global_loss = self.any_loss(t0_F_encoder_T, F_fake_i, "global_topology")
            teacher_local_loss = self.any_loss(t0_F_encoder_T, F_fake_i, "eigenvector").item()
            ...
            # Logging.
            loss = {}
            loss['Teacher/loss'] = teacher_loss.item()
            loss_teachers_plot.append(teacher_loss)
            ...
            if (i + 1) % self.opts.model_save_step == 0:
                Teacher_path = os.path.join(self.opts.checkpoint_dir,
                                            'teacher-{}-{}-{}.ckpt'.format(fold, i + 1,self.opts.local_topology_loss_weight))
            ...
            if i == (self.opts.num_iters - 1):
                ...
                for j in range(start_iters, self.opts.num_iters):
                    ...
                    student_loss_2 = self.any_loss(embedding_student, embedding_teacher, "global_topology")
                    ...
                    if j == (self.opts.num_iters - 1):
                        ...
                        epochs = []
                        counter = 1
                        for i in loss_teachers_plot:
                            epochs.append(counter)
                            losses.append(i.detach().item())
                            counter += 1
                        fig = plt.figure()
                        plt.plot(epochs, losses, '-b', label="Teacher")
                        plt.xlabel('Epoch')
                        plt.ylabel('Loss')
                        plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))
                        plot_path = os.path.join(self.opts.plot_dir, "generatorLoss_{}_{}.png".format(
                        ...
                        epochs = []
                        counter = 1
                        for i in loss_embedding_plot:
                            epochs.append(counter)
                            losses.append(i.detach().item())
                            counter += 1
                        plt.plot(epochs, losses, '-r', label="Embedding")
                        plt.xlabel('Epoch')
                        plt.ylabel('Loss')
                        plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))
                        plot_path = os.path.join(self.opts.plot_dir, "embeddingLoss_{}_{}.png".format(
                            fold, self.opts.local_topology_loss_weight))
                        plt.savefig(plot_path, bbox_inches='tight')
                    ...
                    if j == (self.opts.num_iters - 1):
                        ...
                        plt.ylabel('Loss')
                        plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))
                        plot_path = os.path.join(self.opts.plot_dir, "studentLoss_{}_{}.png".format(
                            fold, self.opts.local_topology_loss_weight))
                        plt.savefig(plot_path, bbox_inches='tight')
    ...
    def test(self, real_morphological_loader, real_functional_loader, i):
        ...
        dataframe.to_csv(r'%s/results/teacher_predicted_functional_graphs_%d_%f.csv' % (self.opts.result_root, i, self.opts.local_topology_loss_weight), sep=' ', header=True,
            float_format='%.6f', index=False)

        print("saving TS prediction into csv file...")
        f = predicted_F_student.detach().cpu().numpy()
        dataframe = pd.DataFrame(data=f.astype(float))
        dataframe.to_csv(r'%s/results/student_predicted_functional_graphs_%d_%f.csv' % (self.opts.result_root, i, self.opts.local_topology_loss_weight), sep=' ',
            header=True,
            float_format='%.6f', index=False)
