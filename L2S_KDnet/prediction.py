...
class L2S_KDnet(object):
    ...
    def build_model(self):
        ...
        self.encoder = GCNencoder(self.opts.in_feature_t0, self.opts.hidden1, self.opts.hidden2, self.opts.dropout).to(self.device)
        self.discriminator = Discriminator(self.opts.hidden2, 1, self.opts.dropout).to(self.device)
        self.generator = GCNdecoder(self.opts.hidden2, self.opts.hidden3, self.opts.SRout, self.opts.dropout).to(self.device)

        ## Student model
        self.Student = Student(self.opts.in_feature_t0, self.opts.hidden1, self.opts.hidden2, self.opts.hidden3,
                               self.opts.SRout, self.opts.dropout).to(self.device)

        ## initializations of optimizers
        self.d_optimizer = torch.optim.Adam(self.discriminator.parameters(), self.opts.d_lr,[self.opts.beta1, self.opts.beta2])
        self.g_optimizer = torch.optim.Adam(self.generator.parameters(), self.opts.g_lr,[self.opts.beta1, self.opts.beta2])
        self.e_optimizer = torch.optim.Adam(self.encoder.parameters(), self.opts.g_lr,[self.opts.beta1, self.opts.beta2])
    ...
    def train(self, fold):
        ...
        for i in range(start_iters, self.opts.num_iters):
            ...
            if (i + 1) % self.opts.model_save_step == 0:
                generator_path = os.path.join(self.opts.checkpoint_dir,
                                              'generator-{}-{}-{}-{}.ckpt'.format(fold, i + 1,
                                                                                  self.opts.local_topology_loss_weight,
                                                                                  self.opts.teacher_loss_weight))
                torch.save(self.generator.state_dict(), generator_path)

                encoder_path = os.path.join(self.opts.checkpoint_dir,
                                            'encoder-{}-{}-{}-{}.ckpt'.format(fold, i + 1,
                                                                              self.opts.local_topology_loss_weight,
                                                                              self.opts.teacher_loss_weight))
    ...
    def test(self, real_morphological_loader, real_functional_loader, i):
        ...
        with torch.no_grad():
            ...
            # generates output from student
            embedding_student, predicted_F_student = self.Student(t0_M_encoder_T, adj)

        # generates matrices for visualization
        self.show_mtrx(to_2d(predicted_F_student[0].detach().numpy(), 160), str("prediction" + str(i)))
        self.show_mtrx(to_2d(t0_F_encoder_T[0].detach().numpy(),160), str("ground-truth" + str(i)))
        self.show_mtrx(to_2d((predicted_F_student[0].detach().numpy()) - t0_F_encoder_T[0].detach().numpy(),160), str("residual" + str(i)))
